...
class WritingTreeMixin:
    ...
    def _normalize_fixed_root_order(cls, root_order):
        return [
            cls.LEGACY_ROOT_LABELS.get(name, name)
            for name in (root_order or [])
        ]

    def _apply_saved_root_order(self, root_order):
        """Apply logical 메인 child order while keeping fixed root invariants."""
        preferred = {
            unicodedata.normalize("NFC", str(name)).casefold(): index
        ...
        def sort_key(index_and_item):
            ...
            names = {
                unicodedata.normalize("NFC", item.text(0)).casefold(),
                unicodedata.normalize(
                    "NFC", os.path.basename(rel_path)
                ).casefold(),
            }

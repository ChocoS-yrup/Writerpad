...
class SyncV2Store:
    ...
    def enqueue(
        ...
        with self._transaction() as connection:
            ...
            # Keep only the newest unsent snapshot behind an in-flight/failed
            # operation. Never rewrite an operation that has already been
            # attempted: its operation_id may have committed on the server even
            # if the client only saw a timeout.
            replaceable = connection.execute(
                """
                SELECT * FROM sync_operations
                WHERE document_id = ? AND status = 'pending' AND attempts = 0
                ORDER BY queue_id DESC LIMIT 1
                """,
                (document["document_id"],),
            ).fetchone()
            if (
                replaceable
                and replaceable["relative_path"] == relative_path
                and bool(replaceable["is_deleted"]) == bool(is_deleted)
            ):
                connection.execute(
                    """
                    UPDATE sync_operations
                    SET local_path = ?, content = ?, last_error = '', updated_at = ?
                    WHERE operation_id = ?
                    """,
                    (
                        local_path,
    ...
    def counts(self, local_key=None):
        ...
        with self._reader() as connection:
            ...
            documents = connection.execute(
                f"SELECT COUNT(DISTINCT document_id) AS count FROM sync_operations WHERE {where}",
                params,
            ).fetchone()

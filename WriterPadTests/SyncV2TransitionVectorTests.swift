...
final class SyncV2TransitionVectorTests: XCTestCase {
    ...
    private func runVector11() throws -> SyncV2VectorRunner {
        let vector = try SyncV2TransitionVector.load("11-cancellation-event-derivation")
    ...
    func testVector11MechanicalExpectations() throws {
        let runner = try runVector11()
        runner.assertMechanicalExpectations()
    }

    /// 같은 사건 식별자로 다시 취소해도, 새 식별자로 다시 취소해도 취소 사건은
    /// 하나뿐이어야 한다. 벡터의 "one cancellation event exists for the first
    /// operation"을 손으로 옮긴 판정이다.
    func testVector11AppendsExactlyOneCancellationEvent() throws {
        let runner = try runVector11()
        let firstOperation = UUID(uuidString: "20000000-0000-4000-8000-000000000081")!

        let cancellations = runner.queue.events(of: firstOperation)
    ...
    func testVector11StateIsReproducibleFromEvents() throws {
        let runner = try runVector11()
        let firstOperation = UUID(uuidString: "20000000-0000-4000-8000-000000000081")!

        let events = runner.queue.events(of: firstOperation)
        XCTAssertEqual(
    ...
    func testVector11CompletedOperationStaysCompleted() throws {
        let runner = try runVector11()
        let secondOperation = UUID(uuidString: "20000000-0000-4000-8000-000000000082")!

        XCTAssertEqual(try runner.queue.state(of: secondOperation), .completed)
        XCTAssertFalse(
            runner.queue.events(of: secondOperation).contains { $0.type == .cancelRequested }
        )
    }

    /// 서버 쪽 기대다. 취소된 쪽은 리비전이 그대로고, 커밋된 쪽만 하나 오른다.
    /// 벡터의 entity_assertions 두 문장을 손으로 옮긴 판정이다.
    func testVector11ServerRevisions() throws {
        let runner = try runVector11()
    ...
    private final class BrokenQueue: SyncV2VectorQueue {
        ...
        func attemptCount(of operationID: UUID) -> Int { attempts[operationID] ?? 0 }
        func events(of operationID: UUID) -> [SyncV2OperationEvent] { eventLog[operationID] ?? [] }
    }

    /// 틀린 구현을 넣으면 하네스가 어긋남을 집어내야 한다. 이 시험이 없으면
    /// 벡터가 통과했다는 말이 아무것도 보증하지 않는다.
    func testHarnessCatchesBrokenImplementation() throws {
        let vector = try SyncV2TransitionVector.load("11-cancellation-event-derivation")
        let client = try XCTUnwrap(vector.initialClientStates.first)
        let runner = SyncV2VectorRunner(vector: vector, queue: BrokenQueue(clientState: client))
        try runner.run()

        let mismatches = runner.mechanicalMismatches()
    ...
    func testReferenceQueueHasNoMismatch() throws {
        XCTAssertEqual(try runVector11().mechanicalMismatches(), [])
    }
}

...
final class AppEnvironmentTests: XCTestCase {
    ...
    func testEditorSessionRefreshesStatisticsDuringContinuousTyping() async throws {
        ...
        for count in 1...9 {
            ...
            // 계산만으로 정해진다.
            let didSuspend = await clock.waitForSleep(after: sleeps)
            let woken = clock.advance(by: .milliseconds(45))
            wakeCount += woken
            if woken > 0 || !didSuspend {
...
private final class VirtualStatisticsClock: @unchecked Sendable {
    ...
    /// 예약 자체가 오지 않는 결함일 때 시험이 영영 멈추지 않도록 두는 한계다.
    /// 정상 경로에서는 곧바로 등록되므로 판정에 끼어들지 않는다.
    private static let registrationBackstop = Duration.seconds(10)
    ...
    /// `previous` 이후로 새 `sleep` 호출이 접수될 때까지 기다린다. 반환값이
    /// true면 그 대기가 등록되어 `advance`로 깨울 수 있다는 뜻이고, false면
    /// 지연이 0이라 대기 없이 지나갔다는 뜻이다. 한 번에 대기 하나만 두는
    /// 호출자를 전제로 마지막 `sleep`의 결과를 돌려준다.
    func waitForSleep(after previous: Int) async -> Bool {
        if let didSuspend = sleepOutcome(after: previous) { return didSuspend }
        ...
        await withCheckedContinuation {
            ...
            Task {
                try? await ContinuousClock().sleep(for: Self.registrationBackstop)
                self.resumeSleepObserver(id)
            }
        }
        return sleepOutcome(after: previous) ?? false
    }

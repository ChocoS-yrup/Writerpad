...
사용자 결정에 따라 계약·Windows 동작·교차 플랫폼 입력이 바뀌지 않는 iPad 전용
변경은 Windows 회신을 기다리지 않고 진행한다. 관련 구현·테스트·문서는 한 PR로 묶고
최종 head에서 한 번만 검토를 요청한다. 실제 교차 플랫폼 차이나 Windows 의존성이
있을 때만 Windows 검토를 요청한다. 아래 상대 플랫폼 인계·검토·draft 유지 규칙은
이 조건에 해당할 때 적용한다. iPad 전용 PR도 로컬 회귀·공통 계약 CI·최종 head 검토와
병합 전 검증은 생략하지 않으며, 인계표의 상대 플랫폼 요청은 `none (iPad-only)`로 기록한다.

...
후속 iPad 구현은 [진단 실행 격리·사전 검사](SyncV2RecoveryRunPreflight.md)에 정리한다.
아래 기존 기능 표와 1회 소비 설명은 위 기준 main의 조사 기록이다. 후속 실행별 명세는
과거 형식을 유지하면서 run UUID별 소비 기록을 추가하며 실제 서버 실행 허가를 열지 않는다.
...
표의 attempts는 `NormalEditorJournal.Save.attempts`의 전송 기록이며 SQLite claim
시도 수나 전체 HTTP 요청 수와 구분한다.

### 각 사건의 공통 절차

1. Git/build/contract와 시험 명세를 고정하고 오프라인 차단 검사를 통과한다.
   원본 앱 데이터의 보관 사본과 시험 전 로컬/서버 기준선을 확보한다.
2. 아래 **사건별 준비**를 적용한다. iPad 본문 저장/source 확인은 A~C 전용이며
   D에는 적용하지 않는다.
...
5. 다시 준비한 뒤 표의 동작만 실행하고 아래 **사건별 증거**를 대조한다.
6. 원래 사건/서버 상태를 보존한다. 성공 뒤에도 원본 fixture나 receipt를 삭제·초기화하지
   않는다. 다음 시험은 이전 사건의 미확정 요청이 남아 있으면 진행하지 않는다.

### A~C: 송신 준비와 증거

- 준비: iPad에서 합성 본문을 한 번 저장하고 영구 source/저널 기록을 확인한다.
  화면에 보이는 글자만으로 저장됐다고 판단하지 않는다. 저장한 본문 SHA-256과
  A~C의 해당 point를 묶어 중단을 활성화한다.
- 증거: request/source의 batch·operation ID, UTF-8 바이트/해시, Base,
  저널 전송 attempts와 서버 receipt의 작성 주체·요청/응답 해시를 대조한다.
  복구 완료 이후 후속 합성 저장이 확인된 새 Base를 쓰는지 별도 확인한다.

### D: 수신 준비와 증거

- 준비: 별도 승인된 상대측 작업으로 원격 합성 본문을 준비한다. 이 문서 수정은
  그 작업을 지금 실행하라는 요청이 아니다. iPad에서는 새 본문을 저장하지 않는다.
  최초 진입에는 진행 중인 수신 건이 없어야 하며, 준비한 remote revision은 로컬 기준선보다
  커야 한다. 같은 revision의 변경 없음 경로를 D 중단 재현으로 집계하지 않는다.
  로컬 본문과 기존 기준선의 UTF-8 바이트/해시 일치, dirty·조합·초안 영구화 오류 없음,
  `state.head == nil`, 충돌·저널 오류 없음을 확인한다. 준비 조건을 만족하지 못하면
  시험을 중단하며, 기존 송신을 먼저 완료하거나 저널을 편집해 조건을 맞추지 않는다.
- 중단 설정: 저장할 로컬 본문이 아니라 **들어올 remote 본문**의 SHA-256과
  `afterOriginalApply`를 지정한다. 수신 동작으로 remote를 저널에 보존하고
  원본 TXT를 적용한 뒤 중단 지점에 도달하는지 확인한다.
- 중단 증거: `journal.receive`의 수신 건 ID, baseline/remote의 revision·본문 해시,
  `originalApplyStarted` phase, 실제 원본 TXT와 초안 상태를 기록한다. 이 지점에서는
  원본은 remote 본문이고 기준선은 이전 값일 수 있으므로 완료 후 일치 조건과 구분한다.
- 복구 증거: 재준비 후 **수신**으로 같은 보관 수신 건을 마무리한다. 중단 전 수신 ID와
  저장된 remote의 연속성을 확인하고, 완료 뒤 원본·기준선·초안이 해당 remote 본문과
  바이트/해시로 일치하며 기준선 revision이 remote와 같고 `journal.receive`가 비워졌는지
  확인한다. D 자체에는 송신 batch·operation·attempts·receipt를 필수 증거로 요구하지 않는다.
  상대측 원격 변경의 receipt가 있다면 별도 생산자 근거로 연결한다.
...
계정/작품/문서 대응, contract SHA, 본문 길이·해시, 실행 시각, 사용자 조작과 직접 관측의
구분을 남긴다. A~C는 요청 ID·해시, 중단 전후 phase·Base·attempts·queue 상태와 서버
receipt를 기록한다. D는 수신 ID, baseline/remote revision·해시, 수신 phase와 원본/초안
상태를 기록하며, 상대측 receipt는 별도 생산자 근거로 구분한다.
공개 인계에는 자격 증명, 원본 DB/원고, 실제 기기 UDID, 설치 컨테이너 ID를 넣지 않는다.
사건에 적용되지 않는 필드는 `해당 없음`으로 표시한다. 필요한 필드를 수집하지 못한 경우
0 또는 통과로 채우지 말고 `미수집`/`미판정`으로 남긴다.
...
이 계획은 PR #41에서 Windows 재검토 후 병합했다. 다음 순서는 진단 실행별
격리와 fail-closed 준비 검사 구현/오프라인 회귀 → 대상·계정·설치 계획 확정 → A~C 실행이다.
이후 iPad 전용 변경은 사용자 결정에 따라 Windows 회신 없이 진행하고, 실제 교차 플랫폼
차이 또는 Windows 의존성이 있을 때만 최종 head의 Windows 검토를 요청한다.

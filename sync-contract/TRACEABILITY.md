# Sync Contract 0.3.0 blocker traceability
...
| C-05 normalized storage name | `storage_name_normalization`: frozen baseline/exclusion/casefold tables, pre-NFKC supplementary adjacency rejection, post-NFKC separator and defensive baseline checks, trailing-space/dot policy, reserved basenames, exact UTF-8 comparison | `protocol.schema.json` storage-name-v2 rule and `storage-name-vectors.schema.json` | SN-001 through SN-029 cover released frozen-data paths; the defensive post-NFKC baseline branch is currently unreachable with those assets | `verify_frozen_unicode_assets` checks all four canonical table digests; `verify_storage_vectors` independently computes every normative vector without host casefold; `verify_defensive_post_normalization_baseline` narrows only a synthetic baseline so U+AB70 casefolds to out-of-baseline U+13A0 and must return `STORAGE_NAME_UNASSIGNED` |
...
- The read-only iPad device scan is reachable from main at merge commit
  `76b9862078dca4571882ec69a5afd9e88fa9bd4e`; it closes only the existing-name
  migration precondition and does not prove a server or client implementation.

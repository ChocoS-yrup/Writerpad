...
-- A project row can survive while its owner membership is missing (for
-- example after an interrupted/manual cleanup). The owner recorded on the
-- project may safely restore only their own membership. Other users still
-- receive FORBIDDEN and cannot claim an existing project UUID.

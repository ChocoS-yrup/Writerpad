...
class WritingModeWidget(WritingTreeMixin, WritingExtractionMixin, QWidget):
    ...
    def update_storage_status(self, state, detail="", pending_count=0):
        labels = {
            ...
            "backup": ("● 저장 중", "#7dd3fc", "#173449"),
            "syncing": ("● 저장 중", "#7dd3fc", "#173449"),
        ...
        editor_dirty = WritingModeWidget._editor_has_unsaved_changes(self)
        if editor_dirty:
            dirty_suffix = {
                "offline": " · 오프라인",
                "lease": " · 다른 기기 편집 중",
        ...
        if editor_dirty:
            tooltip = (
                "현재 에디터 내용이 변경되었습니다. "
                "입력을 멈추면 자동저장되며 Ctrl+S로 즉시 저장할 수 있습니다."
            )
            if detail:
                tooltip += f"\n{detail}"
        else:
        ...
        if pending_count:
            ...
            elif state in {"project_trashed", "project_purged"}:
                tooltip += "\n대기 중인 변경은 로컬에 보존되며 서버로 재전송하지 않습니다."
            else:
                tooltip += "\n클릭하면 지금 재시도합니다."
    ...
    def _editor_has_unsaved_changes(target):
        if bool(
            getattr(target, "is_dirty_left", False)
            or getattr(target, "is_dirty_right", False)
        ):
            return True
        for editor_name in ("left_editor", "right_editor"):
